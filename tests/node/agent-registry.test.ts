...
function agent(overrides: Partial<AgentRecord> = {}): AgentRecord {
  return {
    ...
    operatorWallet: OPERATOR.toLowerCase(),
    payoutWallet: OWNER.toLowerCase(),
...
test("wallet comparison is case-insensitive", () => {
  const verdict = verifyChallenge({
    challenge: challenge({ wallet: OPERATOR.toLowerCase() }),
    recoveredWallet: OPERATOR.toUpperCase(),
    expectedChainId: CHAIN,
    usedNonces: new Set(),
    now: NOW,
  });
...
test("a challenge for another chain is refused", () => {
  ...
  assert.equal(verdict.ok, false);
  assert.equal(verdict.reason, "wrong_chain");
});

test("a signature from a different wallet is refused", () => {
  const verdict = verifyChallenge({
    challenge: challenge(),
    recoveredWallet: STRANGER,
    expectedChainId: CHAIN,
    usedNonces: new Set(),
    now: NOW,
  });
...
test("only the owner may rotate the operator key", () => {
  ...
  assert.equal(byOwner.ok && byOwner.agent.operatorWallet, STRANGER.toLowerCase());
  // The owner and the fee destination are untouched by a rotation.
  assert.equal(byOwner.ok && byOwner.agent.ownerWallet, OWNER.toLowerCase());
  assert.equal(byOwner.ok && byOwner.agent.payoutWallet, OWNER.toLowerCase());
});
