...
test("snapshot addresses are normalised", () => {
  const frozen = snapshotFeePolicy(
    policy({ platformRecipient: PLATFORM.toUpperCase(), agentOwnerRecipient: AGENT_OWNER.toUpperCase(), agentOwnerFeeBps: 100 }),
  );
  assert.equal(frozen.platformRecipient, PLATFORM.toLowerCase());
  assert.equal(frozen.agentOwnerRecipient, AGENT_OWNER.toLowerCase());
});
...
test("one address receiving both fees is merged, not double-counted", () => {
  ...
  assert.equal(merged[0]!.recipient, PLATFORM.toLowerCase());
});
